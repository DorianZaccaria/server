...
ulint buf_page_compress_fcrc32_get_data_size(const byte* buf)
{
	...
	ulint actual_size = (ptype & ~(1UL << FIL_PAGE_COMPRESS_FCRC32_MARKER));
	actual_size = actual_size >> 5;

	ut_ad(actual_size > 0);

	return (actual_size * 64);
}
...
ulint buf_page_compress_fcrc32_get_size(const byte* buf)
{
	...
	return (buf_page_compress_fcrc32_get_data_size(buf)
		+ FIL_PAGE_COMP_ALGO);
}
